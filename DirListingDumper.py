...
def download_files(url):
    ...
    pathlib.Path(f"{system_path}/{host_directory}/{directory_to_create_on_system}").mkdir(parents=True, exist_ok=True)
    i = 0
    while i <= 4:
        try:
            ...
            i+=1
            print(f"{bcolors.OKBLUE}[❌]{bcolors.ENDC} {bcolors.CYELLOW}{system_path}/{host_directory}/{directory_to_create_on_system}/{bcolors.ENDC}{bcolors.BOLD}{bcolors.OKBLUE}{file_name}{bcolors.ENDC}".replace("//", "/"))
            continue

        if response.status_code == 200:
            with open(f"{system_path}/{host_directory}/{directory_to_create_on_system}/{file_name}", "wb") as f:
            ...
            print(f"{bcolors.OKBLUE}[📥]{bcolors.ENDC} {bcolors.CYELLOW}{system_path}/{host_directory}/{directory_to_create_on_system}/{bcolors.ENDC}{bcolors.BOLD}{bcolors.OKBLUE}{file_name}{bcolors.ENDC} - {bcolors.OKGREEN}{len_str}{bcolors.ENDC}".replace("//", "/"))
            return 0
        else:
            i+=1
            print(f"{bcolors.OKBLUE}[❌]{bcolors.ENDC} {bcolors.CYELLOW}{system_path}/{host_directory}/{directory_to_create_on_system}/{bcolors.ENDC}{bcolors.BOLD}{bcolors.OKBLUE}{file_name}{bcolors.ENDC}".replace("//", "/"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Dump directory listing : python3 DirListingDumper.py http://dirlisting.com /tmp/"
    )
    parser.add_argument(
        "website",
        help='URL where the directory listing is located.',
    )
    parser.add_argument("system_path", help="System file where the dump is saved.")
    ...
    print(f"{bcolors.OKBLUE}[⌛]{bcolors.ENDC} Execution time: {bcolors.GREYBG}{time.time() - without_threads_start}{bcolors.ENDC} secs.")
